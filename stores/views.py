...
def index(request):
    ...
    product = Product.objects.all().order_by('-created_at')[:5]
    context = {
        'sliders': sliders,
        'category': category,
        'product': product 
...
def product(request,id):
    ...
    return render(request, 'stores/single_product.html', context)
...
def add_to_cart(request,id):
    ...
    if cart_id:
        ...
        this_product_in_cart = cart_item.cartitem_set.filter(product= cart_product)
